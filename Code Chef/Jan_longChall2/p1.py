...
def solver():
    ...
    if t != -1:
        ...
        dsa = total(dragon[0], sloth[0])
        if dsa != -1:
            if dsa == dragon[0]: ans = "Dragon"
            else: 
                ans = "Sloth"
            return ans
        else:
            toc = total(dragon[1], sloth[1])
            if toc != -1:
                if toc == dragon[1]: ans = "Dragon"
                else: ans = "Sloth"
                return ans
            else: 
                dm = total(dragon[2], sloth[2])
                if dm != -1:
                    if toc == dragon[2]: ans = "Dragon"
                    else: ans = "Sloth"
                else: ans = "TIE"
                return ans
